...
const darOk = (ejecucion, delivery, hidden_delivery, viajes, repartoCliente, hd, importe, ar1, ar2, r) => {
	if (ejecucion == true) {
		const statuPUT = async (id, name) => {
			let da = await fetch(`http://localhost:3000/statu/${id}`, {
				method: "PUT",
				body: `{"statu": "entregado_${name}"}`,
		...
		hidden_delivery.forEach((hidden) => {
			const setPetition = () => {
				return new Promise((resolve, reject) => {
					...
					viajes += parseInt(hidden.children[4].textContent)
					resolve(statuPUT(hidden.children[0].textContent, delivery))
				})
			}
			setPetition().then(() => window.location.reload())
		})
...
const cerrarSesion = async () => {
	...
	if (option == true) {
		...
		let pedidosDELETE = await fetch('http://localhost:3000/pedidos', {
			method: "DELETE"
		})
		...
		localStorage.clear();
		setTimeout(() => { window.location.href = "puntoVenta.html" }, 2010)
	}
}
const ticketCosina = (numero, clienteP, detalle) => {
	...
	detalleP.innerHTML = detalle.toLowerCase()
	$(nro).css({
		"border": "solid 1px",
		"border-radius": "40%",
		"font-size": "17px",
