...
console.log(colorHamPapas)
	...
	if (milanesasLS == null) {
		milanesasdb = await fetch("http://localhost:3000/milanesas"); milanesasdb = await milanesasdb.json();
		localStorage.setItem("milanesasLS", JSON.stringify(milanesasdb));
	}
	else milanesasdb = JSON.parse(milanesasLS);

	if (lomitosLS == null) {
		lomitosdb = await fetch("http://localhost:3000/lomitos"); lomitosdb = await lomitosdb.json();
	...
	if (hamburguesasLS == null) {
		hamburguesasdb = await fetch("http://localhost:3000/hamburguesas"); hamburguesasdb = await hamburguesasdb.json();
	...
	if (pizzasLS == null) {
		pizzasdb = await fetch("http://localhost:3000/pizzas"); pizzasdb = await pizzasdb.json();
		localStorage.setItem("pizzasLS", JSON.stringify(pizzasdb));
	}
	else pizzasdb = JSON.parse(pizzasLS);

	if (papasLS == null) {
		papasdb = await fetch("http://localhost:3000/papas"); papasdb = await papasdb.json();
		localStorage.setItem("papasLS", JSON.stringify(papasdb));
	}
	else papasdb = JSON.parse(papasLS);

	if (platosLS == null) {
		platosdb = await fetch("http://localhost:3000/platos"); platosdb = await platosdb.json();
	...
	if (bebidasLS == null) {
		bebidasdb = await fetch("http://localhost:3000/bebidas"); bebidasdb = await bebidasdb.json();
	...
	$("#MilanesaNapolitanaEnsa").click((e) => {
		selectComida(0, platosdb[5].nombre, platosdb[5].precio, e.target.classList[1])
	}).addClass(colorPapas)

	$("#MilanesaEnsa").click((e) => {
		selectComida(0, platosdb[6].nombre, platosdb[6].precio, e.target.classList[1])
	}).addClass(colorPapas)
	...
	const enviarData = async () => {
		...
		let petiPOST = await fetch("http://localhost:3000/pedidos", {
			method: "POST",
			body: JSON.stringify(pedidoFinal),
			headers: { "content-type": "application/json" }
		})
		let dataPOST = await petiPOST.json();
		let petiGET = await fetch("http://localhost:3000/pedidos")
				...
				if (pedidoFinal.pedido.length != 0) {
					...
					if (edit != null) {
						const peticionPut = async () => {
							...
							const resPut = await fetch(`http://localhost:3000/pedidos/${JSON.parse(edit).id}`, {
								method: "PUT",
								body: JSON.stringify(pedidoFinal),
								headers: { "content-type": "application/json" }
							})
